 ...
 * (user.github.io/psyq-asm/) rather than only from a domain root.
 */
...
export function assembleSite(outDir) {
  ...
  const copied = readdirSync(join(ROOT, 'dist'), { recursive: true, encoding: 'utf8' }).filter(
    (name) => name.endsWith('.js'),
  );
  for (const name of copied) {
    mkdirSync(dirname(join(site, 'dist', name)), { recursive: true });
    cpSync(join(ROOT, 'dist', name), join(site, 'dist', name));
  }
